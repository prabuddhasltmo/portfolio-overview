...
import { generateAIDraft, sendMessage } from '../../services/emailService';
import {
  EmailPriority,
  type EmailFormData,
...
export default function DraftEmailModal({
  ...
  const generateDraft = useCallback(async () => {
    ...
    setError(null);
    try {
      const draft = await generateAIDraft(context);
      setFormData((prev) => ({
        ...prev,
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          >
            <AutoAwesomeIcon sx={{ fontSize: 20, color: blueColor }} />
          </Box>
      ...
      <DialogContent sx={{ pt: 3.5 }}>
        ...
        >
          ...
          {context.daysPastDue && context.daysPastDue > 0 && (
            <Typography
              sx={{ fontSize: '13px', color: theme.palette.error.main }}
            >
              <strong>{context.daysPastDue} days past due</strong>
            </Typography>
          )}
        </Box>
        ...
        {loading ? (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            >
              <AutoAwesomeIcon
                sx={{ fontSize: 16, color: blueColor, animation: 'pulse 1.5s infinite' }}
              />
          ...
          <>
            ...
            >
              ...
              <Typography sx={{ fontSize: '12px', color: neutral?.[500] }}>
                AI drafted this one-way email based on the loan context. You can edit before sending. Recipients cannot reply to this address.
              </Typography>
            </Box>
          </>
        )}
